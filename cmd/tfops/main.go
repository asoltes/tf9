...
import (
	...
	"github.com/andres/tfops/internal/server"
...
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:           "tfops <terraform-command> [target-filter] [flags] [-- terraform-args]",
	...
	root.PersistentFlags().StringVar(&configPath, "config", "", "Configuration file (default: ~/.config/tfops/config.yaml)")
...
func runTerraform(cmd *cobra.Command, args []string) error {
	...
	if removedCommands[tfCommand] {
		return fmt.Errorf("%q was removed; use tfops config repo or tfops config target", tfCommand)
	}
	...
	// we restore the foreground, any subsequent Ctrl+C reaches tfops here and
